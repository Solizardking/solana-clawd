...
const SlotMachine = forwardRef(({ value }: SlotMachineProps, ref) => {
  ...
  useEffect(() => {
    if (phase === 'idle' && fruit0 !== '' && fruit1 !== '' && fruit2 !== '') {
      ...
      recordAgentOutcome([fruit0, fruit1, fruit2], coinsWon);
  ...
  useEffect(() => {
    if (!agentMode || phase !== 'idle') return;

    const autoSpinTimer = window.setTimeout(() => {
      handleSpinAction();
    }, 900);

    return () => window.clearTimeout(autoSpinTimer);
  }, [agentMode, phase, handleSpinAction]);
