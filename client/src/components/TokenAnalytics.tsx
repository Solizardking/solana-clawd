...
const chartParamsForTimeframe = (timeframe: TokenAnalyticsProps['timeframe']) => {
...
export function TokenAnalytics({ mintAddress, timeframe = '24h', onClose }: TokenAnalyticsProps) {
  ...
  const { data: metadata, isLoading: metadataLoading } = useQuery<TokenMetadata>({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        supply: token.content.token_info?.supply,
        decimals: token.content.token_info?.decimals || 9,
        price: token.content.token_info?.price_info?.price_per_token,
        imageUrl: token.content.files?.[0]?.uri,
  ...
  const { data: chartData, isLoading: chartLoading } = useQuery<NormalizedChartPoint[]>({
    queryKey: ['tokenChart', mintAddress, selectedTimeframe],
    queryFn: async () => {
      const response = await apiRequest(`/api/birdeye/ohlcv/${mintAddress}?${chartParamsForTimeframe(selectedTimeframe)}`);
