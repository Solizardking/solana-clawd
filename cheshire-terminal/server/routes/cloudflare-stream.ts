...
function analyzeHlsPlaylist(sample = "") {
  const lines = sample.split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
  const mediaSegmentCount = lines.filter((line) => !line.startsWith("#") && !line.endsWith(".m3u8")).length;
  const variantPlaylistCount = lines.filter((line) => !line.startsWith("#") && line.endsWith(".m3u8")).length;
  ...
  return {
    isPlaylist: lines[0] === "#EXTM3U",
    mediaSegmentCount,
    variantPlaylistCount,
    targetDuration: targetDurationLine ? Number(targetDurationLine.split(":")[1]) || null : null,
    mediaSequence: mediaSequenceLine ? Number(mediaSequenceLine.split(":")[1]) || null : null,
    playlistType: playlistTypeLine?.split(":")[1] || null,
  };
}
...
router.get("/health/:uid", async (req, res) => {
  ...
  const hlsAnalysis = analyzeHlsPlaylist(hls.sample);
  const liveReady = hls.ok && hlsAnalysis.isPlaylist && hlsAnalysis.mediaSegmentCount > 0;
  delete (hls as { sample?: string }).sample;
