...
const ignoredByGit = loadGitIgnoredEntries();
...
let skippedGitIgnoredEntries = 0;
...
function loadGitIgnoredEntries() {
  try {
    ...
    return output.split("\0").filter(Boolean).map((entry) => entry.replace(/^\.\//, ""));
  } catch {
    return [];
  }
}
...
function isGitIgnored(relativePath) {
  return ignoredByGit.some((entry) => {
    const normalized = entry.endsWith("/") ? entry : `${entry}/`;
    return relativePath === entry || relativePath.startsWith(normalized);
  });
}
...
console.log(`Open-source audit passed. Scanned ${scannedFiles} text files; skipped ${skippedEntries} configured local entries and ${skippedGitIgnoredEntries} gitignored entries.`);
