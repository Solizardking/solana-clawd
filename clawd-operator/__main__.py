#!/usr/bin/env python3
# ABOUTME: CLI entry point for OpenClawd Operator with all wrapper functionality
# ABOUTME: Provides complete command-line interface including init, status, and clean commands

"""Command-line interface for OpenClawd Operator."""
...
def init_project():
    """Initialize a new OpenClawd Operator project."""
    _console.print_status("Initializing OpenClawd Operator project...")
...
""")
    ...
    if not Path("ralph.yml").exists():
        with open("ralph.yml", "w") as f:
            f.write("""# OpenClawd Operator Configuration
# Platform: OpenClawd - autonomous trading agents for Solana DeFi
...
""")
    ...
    _console.print_success("OpenClawd Operator project initialized!")
    _console.print_info("Edit ralph.yml to customize configuration")
    _console.print_info("Edit PROMPT.md to define your task")


def show_status():
    """Show current OpenClawd Operator project status."""
    _console.print_header("OpenClawd Operator Status")
...
def clean_workspace():
    """Clean OpenClawd Operator workspace."""
    _console.print_status("Cleaning OpenClawd Operator workspace...")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="ralph",
        description="OpenClawd Operator - Command AI through the Llobster Legend loop until done",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Commands:
    ralph               Run the orchestrator (default)
    ralph init          Initialize a new OpenClawd Operator project  
    ralph status        Show current OpenClawd Operator status
...
"""
    ...
    # Init command
    subparsers.add_parser('init', help='Initialize a new OpenClawd Operator project')
    
    # Status command
    subparsers.add_parser('status', help='Show current OpenClawd Operator status')
...
---""")
    ...
    try:
        ...
        _console.print_header("Starting OpenClawd Operator")
        _console.print_info("Operator: Llobster Legend | $CLAWD Solana DeFi stack")
        _console.print_info(f"Agent: {config.agent.value}")
        if config.prompt_text:
        ...
        _console.print_separator()
        _console.print_success("OpenClawd Operator completed successfully")
