...
**Problem**: OpenClawd Operator runs maximum iterations without achieving goals
...
OpenClawd Operator's loop detection triggers when agent output is ≥90% similar to any of the last 5 outputs.
...
**Problem**: OpenClawd Operator continues running despite `TASK_COMPLETE` marker
...
**Problem**: OpenClawd Operator consuming excessive memory
...
cat > diagnose.sh << 'EOF'
#!/bin/bash
echo "OpenClawd Operator Diagnostic"
echo "============================"
...
git status --short
echo ""
echo "OpenClawd Operator status:"
./ralph status
echo ""
echo "Recent errors:"
...
1. OpenClawd Operator version: `ralph --version`
2. Agent versions
3. Error messages
4. PROMPT.md content
5. Diagnostic output
