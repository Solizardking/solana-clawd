...
OpenClawd Operator implements a simple yet effective architecture based on the Llobster Legend operator loop - a continuous loop pattern that runs AI agents until task completion.

## Core Components

### 1. Orchestration Engine

The heart of OpenClawd Operator is the orchestration loop in `ralph_orchestrator.py`:
...
OpenClawd Operator supports multiple AI agents through a unified interface:
...
OpenClawd Operator uses Git for:
...
├── .agent/                  # OpenClawd Operator workspace
