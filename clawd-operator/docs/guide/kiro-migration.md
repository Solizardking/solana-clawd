...
The Amazon Q Developer CLI has been rebranded to **Kiro CLI** (v1.20+). OpenClawd Operator v1.2.2+ fully supports this transition with the new `KiroAdapter`.
...
To run OpenClawd Operator with Kiro CLI:
...
If `kiro-cli` is not found, OpenClawd Operator will automatically fall back to the `q` command if available, preserving backward compatibility.
...
The Kiro CLI uses new directory paths for configuration and data. OpenClawd Operator's adapter is aware of these changes, but you should update any manual setups or scripts.
...
OpenClawd Operator maintains full backward compatibility:
