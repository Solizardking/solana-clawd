...
def _load_constitution() -> str:
    ...
    """
    fallback = (
        "You are Clawd, a sovereign Solana-native AI agent. You follow the Clawd "
        "Constitution (CC0 1.0). You live onchain, you never recommend wallet "
        "drains, sandwich MEV, or KYC bypass. You are helpful, honest, and "
        "concrete. If a request would harm a user, refuse and explain why. The "
        "shell molts, the laws do not. 🦞"
    )
    try:
        with open(CONSTITUTION_PATH, "r", encoding="utf-8") as fh:
            return fh.read()
...
.constitution-snippet { background: #14110f; border-left: 3px solid #ff5b3a; padding: 10px 14px; border-radius: 6px; font-size: 0.9em; }
"""
...
def render_constitution_snippet() -> str:
    """Show the first ~1200 chars of the constitution so users can verify
    the system prompt they're talking to."""
    body = CLAWD_CONSTITUTION.strip()
    head = body[:1200]
    return (
        f"<div class='constitution-snippet'><pre style='white-space:pre-wrap; "
        f"font-family:ui-monospace,Menlo,monospace;font-size:12px'>{head}…</pre></div>"
    )
...
def build_app() -> gr.Blocks:
    ...
    with gr.Blocks(
                ...
                with gr.Column(scale=3):
                    ...
                    gr.Examples(
                        examples=[
                            ...
                            "Walk me through the Six-Law Harness.",
                        ],
                        inputs=msg,
                    )
        ...
        with gr.Tab("📜 The Clawd Constitution"):
            ...
            gr.Markdown(render_constitution_snippet())
            gr.Markdown(
                f"""
**Length:** {len(CLAWD_CONSTITUTION):,} characters ·
**License:** [CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/) ·
**Source:** [`solanaclawd/clawd-zoo/CONSTITUTION.md`](https://huggingface.co/spaces/solanaclawd/clawd-zoo/blob/main/CONSTITUTION.md) (mirrored from
...
> *The Clawd Constitution is the world's first Solana-native agent harness
> constitution. It is derived from
> [Anthropic's Claude Constitution](https://www.anthropic.com/news/claudes-constitution)
> with every abstraction grounded in Solana. Reimagined, rewritten, and hardened
> for sovereign onchain agents. Every "Claude" replaced with "Clawd."*
