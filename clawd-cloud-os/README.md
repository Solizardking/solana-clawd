...
## OpenClawd Monorepo Integration

When `clawd-cloud-os` lives inside the OpenClawd repo, it now resolves paths in
this order:

1. `OPENCLAWD_ROOT` if set
2. The parent directory of `clawd-cloud-os`
3. Repo-local `solana-clawd/`
4. Fallback `~/src/solana-clawd`

That means root-level scripts work without copying files into `~/src`:
...
Runtime state is kept outside the repo at `~/.clawd-cloud-os/`:

| Path | Purpose |
| ---- | ------- |
| `~/.clawd-cloud-os/pids/` | Managed service pid files |
| `~/.clawd-cloud-os/logs/` | SolanaOS and MCP logs |
| `clawd-cloud-os/config/clawd-cloud-os.json` | Ports, services, and integration manifest |

Environment files are loaded in precedence order:

1. `clawd-cloud-os/.env`
2. OpenClawd root `.env`
3. `solana-clawd/.env`
...
| `clawd-cli logs [service]` | Show managed service logs |
| `clawd-cli paths` | Show resolved repo, Cloud OS, state, log, and pid paths |
| `clawd-cli env` | Show loaded env files and masked key status |
| `clawd-cli validate` | Validate local Cloud OS configs, shell syntax, and required files |
...
│   └── lib/common.sh      # Shared path, env, pid, log, and health helpers
├── tools/
...
│   ├── clawd-cloud-os.json             # Service + OpenClawd integration manifest
...
export CLAWD_WEB_PORT=3000
```

## Agent Registry Registration

The registry command is now an executable operator flow:
...
| -------- | -------- | ------- |
| `HELIUS_API_KEY` | yes | Mainnet Helius RPC |
| `SOLANA_KEYPAIR_PATH` | yes, unless `SOLANA_PRIVATE_KEY` is set | Wallet keypair JSON path |
| `SOLANA_PRIVATE_KEY` | no | Inline JSON byte array alternative |
| `CLAWD_REGISTRATION_FILE` | no | Registration JSON override |
| `CLAWD_AGENT_METADATA_URI` | no | Agent metadata URI |
