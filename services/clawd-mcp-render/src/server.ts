 ...
 *   stocks      (13) — Massive financial market data, includes stock workflows, requires MASSIVE_API_KEY
...
import { MASSIVE_TOOLS } from "./tools/massive-tools.js";
...
async function buildOrchestrator(
  ...
  orch.registerAll(MASSIVE_TOOLS);
  ...
  reg({ name: "prompt_suggestions", description: "Context-aware suggested next prompts for Solana research, agents, x402, and OODA", inputSchema: { type: "object", properties: {} }, category: "memory" },
    async () => [
      ...
      `### Stocks`, `- "Get AAPL previous close with stock_previous_close"`, `- "Fetch 30 days of AAPL bars with stock_aggregates, then query it with massive_query_data"`,
...
export async function createServer(): Promise<Server> {
  ...
  server.setRequestHandler(ListPromptsRequestSchema, async () => ({
    prompts: [
      ...
      { name: "stock_research", description: "Massive stock data workflow for a ticker", arguments: [{ name: "ticker", description: "Stock ticker, for example AAPL", required: true }] },
      ...
      switch (name) {
        case "clawd_orchestrate":
          return msg([
            ...
            `- stock_aggregates / stock_previous_close / stock_ticker_news → Massive stock market data when MASSIVE_API_KEY is configured`,
            `- massive_search_endpoints / massive_call_api / massive_query_data → composable Massive financial data workflow`,
            ``,
            `## 2. OODA Loop Control`,
