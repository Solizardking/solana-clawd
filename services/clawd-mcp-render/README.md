...
- `integration_status` includes `mcp/clawd-mcp`, `mcp-server`, and the local `packages/*` surfaces: `agent-hub`, `agent-registry`, `agents-x402-solana`, `agentwallet`, `clawd-code-cli`, `clawd-guard`, `clawd-protocol`, `clawd-sdk`, `clawd-wallet`, `cli-standalone`, `percolator`, and `sovereign-research`.
...
### 8. Massive Financial Data (`src/tools/massive-tools.ts`)
Massive financial-market data tools use `MASSIVE_API_KEY`. Stocks are included as a first-class workflow, and the generic Massive tools can discover/call/query the wider Massive REST surface:
- `massive_status` verifies configuration without exposing the key.
- `massive_search_endpoints` / `massive_get_endpoint_docs` discover Massive REST endpoint docs.
- `massive_call_api` calls a Massive API path and can store rows in memory with `store_as`.
- `massive_query_data` queries stored rows with SQL-style `SHOW TABLES`, `DESCRIBE`, `DROP TABLE`, and `SELECT ... WHERE/GROUP BY/ORDER BY/LIMIT` workflows.
- `stock_aggregates`, `stock_previous_close`, `stock_snapshot`, `stock_ticker_details`, `stock_ticker_news`, and `stock_market_status` cover common U.S. stock workflows.
- `massive_labor_market` covers the Fed labor-market endpoint: unemployment, participation, average hourly earnings, and job openings.
- Stored rows support apply helpers such as `sma(close, 20)`, `ema(close, 20)`, `simple_return(close)`, `log_return(close)`, `cumulative_return(close)`, `sharpe_ratio(close)`, `sortino_ratio(close)`, and Black-Scholes `bs_*` functions.
...
| stocks         | 13    | Massive financial data, including stock workflows |
...
| `stock_research` | Massive stock data workflow |
...
```bash
curl -fsSL https://raw.githubusercontent.com/x402agent/solana-clawd/main/mcp/clawd-mcp/install.sh | bash
```

The installer:
- clones or updates `https://github.com/x402agent/solana-clawd.git` into `~/.solana-clawd-mcp/solana-clawd`;
- installs and builds `mcp-server`, `packages/agent-hub`, `packages/agent-registry`, `packages/agents-x402-solana`, `packages/agentwallet`, `packages/clawd-code-cli`, `packages/clawd-guard`, `packages/clawd-sdk`, `packages/clawd-wallet`, `packages/cli-standalone`, `packages/percolator`, and `packages/sovereign-research`;
...
- installs and builds the `mcp/clawd-mcp` package;
- creates launchers in `~/.local/bin`: `solana-clawd-mcp`, `solana-clawd-mcp-http`, `pump-mcp`, `clawd-hub`, `clawd-registry`, `agentwallet`, `clawd`, `clawd-code`, `clawd-leviathan`, `clawd-guard`, `clawd-wallet`, `clawd-standalone`, `percolator`, and `sovereign`;
...
```bash
curl -fsSL https://raw.githubusercontent.com/x402agent/solana-clawd/main/mcp/clawd-mcp/install.sh | bash -s -- --branch=newnew
curl -fsSL https://raw.githubusercontent.com/x402agent/solana-clawd/main/mcp/clawd-mcp/install.sh | bash -s -- --install-dir="$HOME/dev/solana-clawd-mcp"
curl -fsSL https://raw.githubusercontent.com/x402agent/solana-clawd/main/mcp/clawd-mcp/install.sh | bash -s -- --skip-packages
```
...
Massive financial data tools require a Massive API key in the MCP environment:
...
The repo includes a Composio integration in [src/composio](/Users/8bit/Downloads/solana-clawd/mcp/clawd-mcp/src/composio) plus a helper CLI that provisions a Composio-hosted MCP server and prints a Claude/Clawd-compatible `.mcp.json` snippet.
...
Verified on July 4, 2026:
- `npm ci` completed from `mcp/clawd-mcp/package-lock.json`.
- `npm run build` completed successfully.
- `npm run typecheck` completed successfully.
- `npm pack --dry-run` included the rebuilt `dist/` server files.
- `GET /health` returned `status: ok` in HTTP mode on `PORT=3099`.
- MCP `initialize` returned server `solana-clawd` version `3.0.0`.
- The runtime path resolver found repo root `/Users/8bit/Downloads/solana-clawd` and MCP package root `/Users/8bit/Downloads/solana-clawd/mcp/clawd-mcp`.
- The docs system loaded `mcp/clawd-mcp/README.md`, and plugin discovery found `mcp/clawd-mcp/src/composio/clawd-provider.ts`.
