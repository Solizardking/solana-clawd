...
**Agent**
: An AI-powered CLI tool that executes tasks based on prompts. OpenClawd Operator supports Claude, Gemini, and Q agents.
...
**Auto-detection**
: OpenClawd Operator's ability to automatically discover which AI agents are installed and available on the system.
...
**CLI**
: Command Line Interface - the primary way to interact with OpenClawd Operator through terminal commands.
...
: The philosophy that it's better to fail predictably than succeed unpredictably - core to the Llobster Legend operator loop.
...
**Git Integration**
: OpenClawd Operator's use of Git for checkpointing, history tracking, and recovery from failed states.

## I

**Iteration**
: One complete cycle of the OpenClawd Operator loop - executing an agent with the current prompt and processing results.

**Iteration Limit**
: Maximum number of iterations before OpenClawd Operator stops. Default is 100, configurable via `max_iterations`.

## L

**Loop**
: The core OpenClawd Operator pattern - continuously running an AI agent until task completion or limits are reached.
...
**MkDocs**
: Static site generator used for OpenClawd Operator's documentation, configured in `mkdocs.yml`.
...
**Plugin**
: Extension mechanism for adding custom agents or commands to OpenClawd Operator.
...
**Llobster Legend Technique**
: The software engineering pattern of commanding AI agents through the OpenClawd operator loop until the task is done, created by Geoffrey Huntley.
...
**Runtime Limit**
: Maximum execution time in seconds before OpenClawd Operator stops. Default is 14400 (4 hours).
...
**Unpossible**
: Reference to Llobster Legend's quote, embodying the philosophy of achieving the seemingly impossible through persistence.
...
**Working Directory**
: The directory where OpenClawd Operator executes, containing PROMPT.md and project files. Defaults to current directory.

**Workspace**
: The `.agent/` directory containing OpenClawd Operator's operational data including metrics, checkpoints, and archives.

## Technical Terms

**API**
: Application Programming Interface - the interface through which OpenClawd Operator communicates with AI services.
...
**`.agent/`**
: OpenClawd Operator's workspace directory containing all operational data.
...
**`ralph init`**
: Initialize a new OpenClawd Operator project with default structure.
