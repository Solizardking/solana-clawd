...
OpenClawd Operator provides comprehensive monitoring capabilities to track execution, performance, and system health. This guide covers monitoring tools, metrics, and best practices.

## Built-in Monitoring

OpenClawd Operator's monitoring system collects and routes execution data through multiple channels:
...
OpenClawd Operator automatically generates state files in `.agent/metrics/`:
...
### 1. OpenClawd Operator Monitor (Built-in)
...
```bash
# Monitor OpenClawd Operator process
...
```bash
#!/bin/bash
# OpenClawd Operator Monitoring Dashboard
...
def dashboard():
    ...
    <html>
        <head>
            <title>OpenClawd Operator Dashboard</title>
            <script>
        ...
        <body onload="updateMetrics()">
            <h1>OpenClawd Operator Dashboard</h1>
            <pre id="metrics"></pre>
        </body>
    </html>
    ''')
...
def check_errors():
    ...
    if state.get('errors'):
        send_alert(f"OpenClawd Operator encountered errors: {state['errors']}")

    if state.get('iteration_count', 0) > 100:
        send_alert("OpenClawd Operator exceeded 100 iterations")

    if state.get('runtime', 0) > 14400:  # 4 hours
        send_alert("OpenClawd Operator runtime exceeded 4 hours")
```
...
notify-send "OpenClawd Operator Alert" "Task completed successfully"

# Email alert
echo "OpenClawd Operator task failed" | mail -s "OpenClawd Operator Alert" admin@example.com

# Slack webhook
curl -X POST -H 'Content-type: application/json' \
    --data '{"text":"OpenClawd Operator task completed"}' \
...
def analyze_iterations():
    ...
    plt.title('OpenClawd Operator Execution Performance')
