...
DEFAULT_CLAWD_TOKEN = "8cHzQHUS2s2h8TzCmfqPKYiM4dSt4roa3n7MyRLApump"

# Known token mints for labeling
KNOWN_TOKENS = {
    DEFAULT_CLAWD_TOKEN: ("CLAWD", 6),
...
class SolanaClient:
    ...
    def __init__(self, rpc_url: Optional[str] = None):
        ...
        self.clawd_token = os.environ.get("CLAWD_TOKEN", DEFAULT_CLAWD_TOKEN)
        KNOWN_TOKENS.setdefault(self.clawd_token, ("CLAWD", 6))
    ...
    async def _rpc_call(self, method: str, params: list = None) -> dict:
        ...
        max_attempts = max(1, int(os.environ.get("SOLANA_RPC_RETRIES", "4")))
            ...
            try:
                ...
                if "error" in data:
                    ...
                    if (
                        ...
                        and error.get("code") in {-32005, 429}
                    ):
                ...
                if attempt >= max_attempts - 1 or status not in {429, 500, 502, 503, 504}:
                    raise
                retry_after = exc.response.headers.get("retry-after")
                delay = float(retry_after) if retry_after else 0.5 * (2 ** attempt)
                await asyncio.sleep(min(delay, 8.0))

        raise RuntimeError(f"RPC call failed after {max_attempts} attempts: {method}")
    ...
    async def get_token_info(self, mint_address: str) -> dict:
        ...
        return {
            ...
            "top_holders_error": holders_error,
        }
