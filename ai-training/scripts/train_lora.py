...
def main() -> None:
    ...
    print(f"[setup] device={device}  base={base_model}  dataset={dataset_repo}")
    ...
    # ---- Dataset ----
    print(f"[4/6] Loading dataset: {dataset_repo}")
    try:
        ds = load_dataset(dataset_repo)
    except Exception as e:
        print(f"  Hub load failed ({e}), falling back to local data/processed")
        ds = load_dataset("data/processed")
    train_ds = ds[cfg.get("train_split", "train")]
    eval_ds = ds.get(cfg.get("eval_split", "eval"), None)
