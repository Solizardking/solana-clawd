...
const normalizeCanonicalUrls = (value) => {
  if (typeof value === "string") {
    return LEGACY_WWW_HOSTS.reduce((current, legacyHost) => current.replaceAll(legacyHost, HOST), value);
  }
  if (Array.isArray(value)) return value.map(normalizeCanonicalUrls);
  if (value && typeof value === "object") {
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, normalizeCanonicalUrls(item)]));
  }
  return value;
};
...
function registryDocToAgent(doc) {
  const identifier = doc.openclawd?.identifier || doc.registrations?.[0]?.agentId?.replace(/^openclawd:/, "");
  const category = doc.categories?.[0] || "defi";
  const capabilities = doc.openclawd?.capabilities || [];
  const metaplexSkills = doc.metaplex?.programs || deriveMetaplexSkills(capabilities, doc.tags || []);
