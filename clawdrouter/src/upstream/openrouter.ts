...
export async function proxyToOpenRouter(
  ...
  const headers: Record<string, string> = {
    ...
    'HTTP-Referer': siteUrl ?? 'https://x402.wtf',
    'X-OpenRouter-Title': siteTitle ?? 'ClawdRouter',
    // X-Title is also supported for backwards compatibility
    'X-Title': siteTitle ?? 'ClawdRouter',
    // Enable experimental router metadata to capture pipeline decisions
