...
export async function saveWallet(wallet: ClawdWallet, walletPath = join(WALLET_DIR, WALLET_FILE)): Promise<void> {
  ...
  await writeFile(walletPath, JSON.stringify(data, null, 2), { mode: 0o600 });
}

export async function loadWallet(walletPath = join(WALLET_DIR, WALLET_FILE)): Promise<ClawdWallet | null> {
...
export async function loadOrCreateWallet(walletPath = join(WALLET_DIR, WALLET_FILE)): Promise<ClawdWallet> {
  const existing = await loadWallet(walletPath);
