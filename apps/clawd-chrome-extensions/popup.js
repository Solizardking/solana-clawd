...
// ── pAGENT status ──────────────────────────────────────────────────────────
let pagentOnline = false;

async function refreshPagentStatus() {
  try {
    const res = await new Promise((resolve) =>
      chrome.runtime.sendMessage({ type: 'PAGENT_STATUS' }, resolve)
    );
    pagentOnline = !!(res?.online);
  } catch {
    pagentOnline = false;
  }
...
async function sendChatAsAgent(task) {
  const res = await new Promise((resolve) =>
    chrome.runtime.sendMessage({ type: 'EXECUTE_AGENT_TASK', task }, resolve)
  );
  if (!res?.ok) throw new Error(res?.error || 'Agent task failed');
  const r = res.result;
  return r?.data || (r?.success ? 'Task completed.' : 'Task failed.');
}
...
function setupChat() {
  ...
  const runBtn = document.getElementById('chatRunBtn');
  if (runBtn) {
    runBtn.addEventListener('click', () => sendChat(true));
  }

  // Poll pAGENT status every 10 s while chat tab is visible
  refreshPagentStatus();
  setInterval(refreshPagentStatus, 10000);
}

async function sendChat(runAsAgent = false) {
  const input = document.getElementById('chatInput');
