...
#     nemoclawd-start
#     openclawd agent --agent main --local --session-id live
...
if ! command -v tmux > /dev/null 2>&1; then
  ...
  echo "    openclawd agent --agent main --local --session-id live"
  exit 0
fi
...
tmux split-window -h -t "$SESSION" \
  "openshell sandbox connect nemoclawd -- bash -c 'export NVIDIA_API_KEY=$NVIDIA_API_KEY && nemoclawd-start openclawd agent --agent main --local --session-id live'"
