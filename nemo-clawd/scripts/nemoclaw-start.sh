...
PUBLIC_PORT=18789
WORKSPACE_ROOT="${HOME:-/sandbox}/.openclawd/workspace"
...
- NemoClaw vault at `~/.nemoclawd/vault/` for append-only JSONL wallet, trade, and heartbeat logs
...
config_path = os.path.join(home, '.openclawd', 'openclawd.json')
...
import json
import os
path = os.path.expanduser('~/.openclawd/agents/main/agent/auth-profiles.json')
...
import json
import os
path = os.path.expanduser('~/.openclawd/openclawd.json')
try:
...
while time.time() < DEADLINE:
    rc, out, err = run('openclawd', 'devices', 'list', '--json')
    if rc != 0 or not out:
        time.sleep(1)
        continue
    try:
    ...
    has_browser = any((d.get('clientId') == 'openclawd-control-ui') or (d.get('clientMode') == 'webchat') for d in paired if isinstance(d, dict))
    ...
    if pending:
        ...
        for device in pending:
            ...
            if not request_id:
                continue
            arc, aout, aerr = run('openclawd', 'devices', 'approve', request_id, '--json')
            if arc == 0:
                APPROVED += 1
...
openclawd doctor --fix > /dev/null 2>&1 || true
openclawd models set nvidia/nemotron-3-super-120b-a12b > /dev/null 2>&1 || true
write_auth_profile
...
openclawd plugins install /opt/nemoclawd > /dev/null 2>&1 || true

if [ ${#NEMOCLAW_CMD[@]} -gt 0 ]; then
  exec "${NEMOCLAW_CMD[@]}"
fi

nohup openclawd gateway run > /tmp/gateway.log 2>&1 &
echo "[gateway] openclawd gateway launched (pid $!)"
start_auto_pair
print_dashboard_urls
