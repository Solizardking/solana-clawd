...
def restore_into_sandbox(snapshot_dir: Path, sandbox_name: str = "openclawd") -> bool:
    ...
    result = subprocess.run(
        ["openshell", "sandbox", "cp", str(source), f"{sandbox_name}:/sandbox/.openclawd"],
        capture_output=True,
        text=True,
        check=False,
    )
...
def cutover_host(snapshot_dir: Path) -> bool:
    ...
    timestamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
    archive_path = OPENCLAW_DIR.parent / f".openclawd.pre-nemoclawd.{timestamp}"
...
def rollback_from_snapshot(snapshot_dir: Path) -> bool:
    ...
    if OPENCLAW_DIR.exists():
        timestamp = datetime.now(UTC).strftime("%Y%m%dT%H%M%SZ")
        archive_path = OPENCLAW_DIR.parent / f".openclawd.nemoclawd-archived.{timestamp}"
        shutil.move(str(OPENCLAW_DIR), str(archive_path))
