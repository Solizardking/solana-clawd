...
describe("cliStatus", () => {
  ...
  describe("host — sandbox running, inference configured", () => {
    ...
    it("shows running sandbox with uptime in text output", async () => {
      ...
      expect(output).toContain("Name:    openclawd");
      expect(output).not.toContain("inside sandbox");
    });
  ...
  describe("inside sandbox — core bug fix", () => {
    beforeEach(() => {
      vi.mocked(existsSync).mockImplementation((p: string | URL | Buffer) => {
        const path = String(p);
        return path === "/sandbox/.openclawd" || path === "/sandbox/.nemoclawd";
      });
    });
  ...
  describe("inside sandbox — with prior plugin state", () => {
    ...
    it("shows rollback section when migrationSnapshot exists", async () => {
      ...
      expect(output).toContain("openclawd nemoclawd eject");
    });
  ...
  describe("edge cases", () => {
    ...
    it("defaults sandbox name to 'openclawd' when state has none", async () => {
      mockExec({
      ...
      expect(exec).toHaveBeenCalledWith(
        expect.stringContaining("openclawd"),
        expect.anything(),
        expect.anything(),
      );
    });

    it("only detects sandbox via /sandbox/.openclawd", async () => {
      vi.mocked(existsSync).mockImplementation((p: string | URL | Buffer) => {
        return String(p) === "/sandbox/.openclawd";
      });
