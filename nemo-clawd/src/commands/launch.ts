...
export async function cliLaunch(opts: LaunchOptions): Promise<void> {
  ...
  if (hostState.exists && !force) {
    logger.info(
      "Existing OpenClaw installation detected. Consider using 'openclawd nemoclawd migrate' instead.",
    );
    logger.info(
  ...
  const compat = checkCompatibility(blueprint.manifest, openshellVersion, openclawdVersion);
  if (compat.length > 0) {
...
function getOpenclawdVersion(): string {
  try {
    return execSync("openclawd --version", { encoding: "utf-8" }).trim();
  } catch {
    return "0.0.0";
  }
}
