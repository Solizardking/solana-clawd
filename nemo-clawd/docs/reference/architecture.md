...
tags: ["openclawd", "openshell", "sandboxing", "blueprints", "inference_routing"]
content:
  type: reference
...
The plugin is a thin TypeScript package that registers commands under `openclawd nemoclawd`.
...
├── openclawd.plugin.json            Plugin manifest
└── package.json                    Commands declared under openclawd.extensions
```
...
├── policies/
│   └── openclawd-sandbox.yaml       Strict baseline network + filesystem policy
```
...
1. Resolve. The plugin locates the blueprint artifact and checks the version against `min_openshell_version` and `min_openclawd_version` constraints in `blueprint.yaml`.
...
[`ghcr.io/nvidia/openshell-community/sandboxes/openclawd`](https://github.com/NVIDIA/OpenShell-Community)
...
- Network egress is restricted by the baseline policy in `openclawd-sandbox.yaml`.
