...
tags: ["openclawd", "openshell", "sandboxing", "network_policy", "security"]
content:
  type: reference
...
The baseline policy is defined in `nemoclawd-blueprint/policies/openclawd-sandbox.yaml`.
...
* - `nvidia`
  ...
  - `/usr/local/bin/claude`, `/usr/local/bin/openclawd`
  - All methods
...
* - `openclawd_docs`
  - `docs.openclawd.ai:443`
  - `/usr/local/bin/openclawd`
  - GET only

* - `npm_registry`
  - `registry.npmjs.org:443`
  - `/usr/local/bin/openclawd`, `/usr/local/bin/npm`
  - GET only
...
Edit `nemoclawd-blueprint/policies/openclawd-sandbox.yaml` and re-run the onboard wizard:
