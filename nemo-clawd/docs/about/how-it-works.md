...
tags: ["openclawd", "openshell", "sandboxing", "inference_routing", "blueprints", "network_policy", "solana", "wallets", "telemetry"]
content:
  type: concept
...
: The `nemoclawd` CLI is the primary interface. Plugin commands are available under `openclawd nemoclawd` but do not override built-in OpenClaw commands.
...
- The *plugin* is a TypeScript package that powers the `nemoclawd` CLI and also registers commands under `openclawd nemoclawd`.
...
When you run `nemoclawd onboard`, NemoClaw creates an OpenShell sandbox that runs OpenClaw in an isolated container.
...
The sandbox starts with a strict baseline policy defined in `openclawd-sandbox.yaml`.
...
NemoClaw keeps an append-only operator trail under `~/.nemoclawd/vault/`.
