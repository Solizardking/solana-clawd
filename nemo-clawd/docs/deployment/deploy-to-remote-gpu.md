...
- NemoClaw installed locally. Install with `npm install -g @mawdbotsonsolana/nemoclawd`.
...
```console
$ openclawd agent --agent main --local -m "Hello from the remote sandbox" --session-id test
```
