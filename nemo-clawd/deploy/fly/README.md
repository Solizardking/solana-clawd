...
```bash
openclawd config set gateway.mode remote
openclawd config set gateway.remote.url wss://your-app.fly.dev
openclawd config set gateway.remote.token <your-gateway-token>
openclawd health  # verify the connection
```
...
- `.openclawd/` — OpenClaw gateway config, conversation history, context
