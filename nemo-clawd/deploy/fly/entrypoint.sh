...
mkdir -p "${DATA_DIR}/.openclawd/agents/main/agent" \
         "${DATA_DIR}/.openclawd/workspace/skills" \
...
if [ ! -f "${DATA_DIR}/.openclawd/openclawd.json" ]; then
...
path = os.path.join('${DATA_DIR}', '.openclawd', 'openclawd.json')
...
if command -v openclawd &>/dev/null; then
  HOME="${DATA_DIR}" openclawd doctor --fix > /dev/null 2>&1 || true
  HOME="${DATA_DIR}" openclawd plugins install /opt/nemoclawd > /dev/null 2>&1 || true
fi
