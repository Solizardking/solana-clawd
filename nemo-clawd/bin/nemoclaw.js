...
function printSandboxHint(sandboxName, action) {
  ...
  if (lastSandbox && lastSandbox !== sandboxName && registry.getSandbox(lastSandbox)) {
    ...
    console.error(`  Try: nemoclawd ${lastSandbox} ${action}`);
  }
}
...
function createSandboxSshConfig(sandboxName) {
  ...
  const configPath = path.join(os.tmpdir(), `nemoclawd-ssh-${sandboxName}-${Date.now()}.conf`);
...
function validateSandboxEnv(label, envValues, requiredKeys, optionalHint) {
  ...
  console.error("");
  console.error("  Export them in your shell or store them in ~/.nemoclawd/credentials.json.");
  if (optionalHint) {
...
async function deploy(instanceName) {
  if (!instanceName) {
    console.error("  Usage: nemoclawd deploy <instance-name>");
    console.error("");
    console.error("  Examples:");
    console.error("    nemoclawd deploy my-gpu-box");
    console.error("    nemoclawd deploy nemoclawd-prod");
  ...
  const envTmp = path.join(os.tmpdir(), `nemoclawd-env-${Date.now()}`);
  fs.writeFileSync(envTmp, envLines.join("\n") + "\n", { mode: 0o600 });
  run(`scp -q -o StrictHostKeyChecking=no -o LogLevel=ERROR "${envTmp}" ${name}:/home/ubuntu/nemoclawd/.env`);
  fs.unlinkSync(envTmp);

  console.log("  Running setup...");
  run(`ssh -t -o StrictHostKeyChecking=no -o LogLevel=ERROR ${name} 'cd /home/ubuntu/nemoclawd && set -a && . .env && set +a && bash scripts/brev-setup.sh'`);

  if (tgToken) {
    console.log("  Starting services...");
    run(`ssh -o StrictHostKeyChecking=no -o LogLevel=ERROR ${name} 'cd /home/ubuntu/nemoclawd && set -a && . .env && set +a && bash scripts/start-services.sh'`);
  }

  console.log("");
  console.log("  Connecting to sandbox...");
  console.log("");
  run(`ssh -t -o StrictHostKeyChecking=no -o LogLevel=ERROR ${name} 'cd /home/ubuntu/nemoclawd && set -a && . .env && set +a && openshell sandbox connect nemoclawd'`);
}
...
function listSandboxes() {
  ...
  if (sandboxes.length === 0) {
    console.log("");
    console.log("  No sandboxes registered. Run `nemoclawd onboard` to get started.");
    console.log("");
    return;
  }
...
function sandboxSolanaAgent(sandboxName) {
  ...
  runSandboxScript(sandboxName, envValues, "nemoclawd-solana-agent");
}
...
function sandboxPaymentApp(sandboxName) {
  ...
  runSandboxScript(sandboxName, envValues, "nemoclawd-payment-app");
}
...
function sandboxSwarmBot(sandboxName) {
  ...
  runSandboxScript(sandboxName, envValues, "nemoclawd-swarm-bot");
}
...
function sandboxSolanaStack(sandboxName, overrides = {}) {
  ...
  runSandboxScript(sandboxName, envValues, "nemoclawd-solana-stack");
}
...
async function launch(actionArgs = []) {
  ...
  if (!hasTelegramToken) {
    ...
    console.log("  Set TELEGRAM_BOT_TOKEN and rerun `nemoclawd launch` for Telegram narration and bot commands.");
  }
  console.log("");
...
async function quickStartSolana(actionArgs = []) {
  ...
  if (wallet) {
    ...
    console.log("  Wallet: not configured (run nemoclawd onboard)");
  }
  ...
  console.log(`    nemoclawd ${sb.name} solana-bridge    Real-time Telegram wallet narration`);
  ...
  console.log(`    nemoclawd ${sb.name} connect`);
  console.log("");
}
...
function sandboxSolanaBridge(sandboxName) {
  ...
  runSandboxScript(sandboxName, envValues, "nemoclawd-solana-bridge");
}
...
async function walletCommand(actionArgs = []) {
  ...
  switch (sub) {
    ...
    case "list": {
      ...
      console.log("");
      if (wallets.length === 0) {
        console.log("  No wallets found. Run: nemoclawd wallet create");
      } else {
    ...
    default: {
      ...
      console.log("");
      if (!privyConfig) {
        console.log("  Get started: nemoclawd wallet create");
      }
      console.log("");
      break;
    }
...
function showVersion() {
  console.log(`@mawdbotsonsolana/nemoclawd v${getCliVersion()}`);
}
...
function doctor() {
  ...
  if (dockerServer.status === 0 && dockerServer.output) {
    ...
    blockers.push("Start Docker Desktop or Colima before running `nemoclawd onboard`.");
  }
  ...
  if (openshellVersion.status === 0 && openshellVersion.output) {
    ...
    printDoctorLine("warn", "OpenShell", "not installed yet; `nemoclawd onboard` will try to install it");
  ...
  if (sandboxes.length === 0) {
    console.log("  Next step: `nemoclawd launch`");
  } else if (defaultSandbox) {
    console.log(`  Next step: \`nemoclawd launch ${defaultSandbox}\``);
  } else {
    console.log("  Next step: `nemoclawd launch`");
  }
...
function help() {
  ...
  Getting Started:
    nemoclawd doctor                  Verify Docker, OpenShell, wallet, and RPC setup
  ...
  Sandbox Management:
    ...
    nemoclawd <name> telegram-bot     Pump-Fun Telegram monitor + API
  ...
  Policy Presets:
    nemoclawd <name> policy-add       Add network policy preset
    nemoclawd <name> policy-list      List presets (● = applied)

  Deploy:
    nemoclawd deploy <instance>       Deploy to a Brev GPU VM
...
(async () => {
  ...
  // Sandbox-scoped commands: nemoclawd <name> <action>
  ...
  if (allNames.length > 0) {
    ...
    console.error(`  Try: nemoclawd <sandbox-name> connect`);
    console.error("");
  }

  console.error(`  Run 'nemoclawd help' for usage.`);
  process.exit(1);
})();
