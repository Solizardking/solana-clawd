...
3. If exit code 1 (no messages): parse the output JSON. If activeRooms is 0, disable this cron: openclaw cron disable CRON_ID (replace CRON_ID with cronId from ${CONFIG_FILE}). Then notify main session: arena rooms inactive, polling paused. Otherwise do nothing.
