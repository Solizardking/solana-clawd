...
import { analyzeComponent } from './stride.js';
// ── Constants ──────────────────────────────────────────────────────────────
...
function resolveSkillComponentPath(inputPath, slug) {
 ...
 */
export function deriveSkillId(slug, kind, specHash) {
    return crypto
        .createHash('sha256')
 ...
 */
export function computeSpecHash(slugOrPath) {
    ...
    for (const base of bases) {
        for (const c of ['SPEC.md', 'spec.md', 'README.md'].map((name) => path.resolve(base, name))) {
            if (!isPathWithinRoot(c, REPO_ROOT))
                continue;
            if (!fs.existsSync(c))
                continue;
            const realCandidate = fs.realpathSync(c);
            if (!isPathWithinRoot(realCandidate, REPO_ROOT))
                continue;
 ...
 */
export async function runVerificationGate(componentPath, kind) {
...
export function catalogEntryToKind(entry) {
    const cat = (entry.category ?? '').toLowerCase();
    if (cat.includes('agent') || cat.includes('ai'))
        return 'agent';
    if (cat.includes('mcp') || entry.slug.endsWith('-mcp'))
        return 'mcp_server';
    if (cat.includes('plugin'))
        return 'plugin';
    return 'skill';
}
/**
 ...
 */
export async function registerSkill(opts) {
    ...
    if (!verification.passed) {
        throw new Error(`[SkillHub] Registration blocked: ${verification.blocked_reason}\n` +
 ...
 */
export async function bulkImportCatalog(opts) {
    const catalog = loadCatalog();
    const registry = loadRegistry();
    const result = { registered: [], skipped: [], blocked: [] };
...
export function revokeSkill(skillId, authority) {
    assertSafeRegistryKey(skillId);
    if (!/^[0-9a-f]{64}$/.test(skillId))
    ...
    if (entry.authority !== authority)
        throw new Error('Not the skill authority');
    entry.active = false;
    saveRegistry(registry);
}
// ── Content fetcher ────────────────────────────────────────────────────────
async function fetchGitHubContent(repoUrl, branch = 'main', subdirectory = '') {
    ...
    const content = {};
    await Promise.allSettled(filenames.map(async (f) => {
        try {
            const resp = await fetch(`${base}${prefix}/${f}`);
            if (resp.ok)
                content[f] = await resp.text();
        }
        catch { }
    }));
    return content;
}
function buildAnalysisText(manifest, fetched) {
    const parts = [`=== Skill: ${manifest.name} (${manifest.slug}) ===`];
...
export async function createSASAttestation(skillId, entry) {
    const fields = {
    ...
    try {
        ...
        if (sasKey.startsWith('[')) {
            secretKey = Uint8Array.from(JSON.parse(sasKey));
        }
        else {
...
const PENDING_SUBMISSIONS = new Map();
export async function submitSkill(manifest) {
    ...
    if (manifest.source) {
        if (manifest.source.type === 'github') {
            fetched = await fetchGitHubContent(manifest.source.url, manifest.source.branch ?? 'main', manifest.source.subdirectory);
        }
...
// ── Submission queue ────────────────────────────────────────────────────────
export function startSubmission(manifest) {
...
if (process.argv[1] === import.meta.filename || process.argv[1]?.endsWith('skill-hub.ts')) {
    ...
    async function main() {
        ...
        if (cmd === 'register') {
            ...
            const kindArg = (args.find(a => a.startsWith('--kind='))?.split('=')[1] ?? 'skill');
