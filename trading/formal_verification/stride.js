        ...
        detect: (text, lines) => {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (/find_program_address|create_program_address/.test(line) &&
                    !/assert_eq.*bump|require_eq.*bump|bump\s*==/.test(lines.slice(i, i + 5).join('\n'))) {
                    matches.push({ line: i + 1, snippet: line.trim().slice(0, 80) });
                }
            }
            return matches.slice(0, 2);
...
// ---------------------------------------------------------------------------
function readSource(componentPath) {
    const abs = resolve(componentPath);
    if (!isPathWithinRoot(abs, REPO_ROOT))
        return '';
    if (!existsSync(abs))
        return '';
    ...
    function walk(dir, depth = 0) {
        ...
        for (const entry of readdirSync(dir, { withFileTypes: true })) {
            ...
            if (!isPathWithinRoot(full, REPO_ROOT))
                continue;
...
// ---------------------------------------------------------------------------
function checkSirenCompliance(source) {
...
// ---------------------------------------------------------------------------
export function analyzeComponent(componentPath) {
    ...
    const threatScore = Math.max(0, 100 - counts.critical * 25 - counts.high * 10 - counts.medium * 5 - counts.low * 2);
