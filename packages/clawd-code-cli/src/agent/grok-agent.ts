...
import { loadCharacter, buildCharacterPrompt } from "../utils/character-loader.js";
...
export class GrokAgent extends EventEmitter {
  ...
  constructor(
    ...
    if (characterName) {
      const characterResult = loadCharacter(characterName);
      if (characterResult) {
        characterSection = `\n\nCHARACTER PERSONA:\n${buildCharacterPrompt(characterResult.data, characterResult.type)}\n\nAdopt the above persona fully. Stay in character while still using all available tools.`;
      }
    }
    ...
    this.messages.push({
      role: "system",
      content: `You are Clawd Code CLI, a lobster-themed AI terminal operator for coding, system operations, and Solana workflows. You run on Grok/xAI models and help with file editing, shell tasks, MCP tools, and Solana blockchain interactions.${customInstructionsSection}${characterSection}
