...
export function useInputHandler({
  ...
  const commandSuggestions: CommandSuggestion[] = [
    ...
    { command: "/models", description: "Switch active model" },
    { command: "/config", description: "Set API keys / base URLs per provider" },
    { command: "/provider", description: "Show current provider config" },
...
Model Configuration:
  Edit ~/.clawd/user-settings.json and update the "models" array
...
Available models: ${modelNames.join(", ")}`,
    ...
    // /config <provider> <key|baseURL> <value>
    //  e.g. /config openrouter key sk-or-v1-...
    //       /config openrouter baseURL https://openrouter.ai/api/v1
    //       /config ollama baseURL http://localhost:11434/v1
    //       /config grok key xai-...
    //
    // /config add model <name>      adds a model to ~/.clawd/user-settings.json "models" list
    // /config set defaultModel <m>  sets the user-level default model
    if (trimmedInput.startsWith("/config")) {
      ...
      if (parts.length === 1) {
        ...
        for (const p of ["grok", "openrouter", "openai", "ollama", "custom"]) {
          ...
          const keyMask = cfg.apiKey
            ...
            : "(unset)";
          lines.push(`  ${p.padEnd(10)} key=${keyMask}  baseURL=${cfg.baseURL || "(default)"}`);
        }
        lines.push("");
        lines.push("Usage:");
        lines.push("  /config <provider> key <apiKey>");
        lines.push("  /config <provider> baseURL <url>");
      ...
      // /config add model <name>
      if (parts[1] === "add" && parts[2] === "model" && parts[3]) {
        const newModel = parts.slice(3).join(" ");
        const existing = manager.getAvailableModels();
        if (!existing.includes(newModel)) {
          manager.updateUserSetting("models", [...existing, newModel]);
        }
        setChatHistory((prev) => [
          ...prev,
          {
      ...
      // /config set defaultModel <m>
      if (parts[1] === "set" && parts[2] === "defaultModel" && parts[3]) {
        const m = parts.slice(3).join(" ");
        manager.updateUserSetting("defaultModel", m);
        ...
        setChatHistory((prev) => [
          ...
          {
            type: "assistant",
            content: `✓ Default model set to ${m} (switched active session too).`,
            timestamp: new Date(),
          },
        ]);
        clearInput();
      ...
      if (!field || !value) {
        setChatHistory((prev) => [
          ...
          {
            type: "assistant",
            content: `Usage: /config ${provider} key <apiKey> | /config ${provider} baseURL <url>`,
            timestamp: new Date(),
          },
        ]);
        clearInput();
      ...
      if (field === "key" || field === "apiKey") {
        manager.setProviderConfig(provider, { apiKey: value });
      } else if (field === "baseURL" || field === "url") {
        manager.setProviderConfig(provider, { baseURL: value });
      } else {
        setChatHistory((prev) => [
          ...prev,
          {
            type: "assistant",
            content: `Unknown field: ${field}. Use 'key' or 'baseURL'.`,
            timestamp: new Date(),
          },
        ]);
        clearInput();
      ...
      // If the active model is on this provider, rebuild the client immediately.
      const currentModel = agent.getCurrentModel();
      if (manager.getProviderForModel(currentModel) === provider) {
      ...
      setChatHistory((prev) => [
        ...
        {
          type: "assistant",
          content: `✓ Saved ${provider}.${field === "key" || field === "apiKey" ? "apiKey" : "baseURL"}`,
          timestamp: new Date(),
        },
      ]);
      clearInput();
    ...
    if (trimmedInput === "/provider") {
      const manager = getSettingsManager();
      const currentModel = agent.getCurrentModel();
      const cfg = manager.getProviderConfigForModel(currentModel);
      const keyMask = cfg.apiKey
        ? cfg.apiKey.slice(0, 6) + "…" + cfg.apiKey.slice(-4)
        : "(unset)";
      setChatHistory((prev) => [
        ...
        {
          type: "assistant",
          content: `Active model: ${currentModel}\nProvider : ${cfg.provider}\nBase URL : ${cfg.baseURL}\nAPI key  : ${keyMask}`,
          timestamp: new Date(),
        },
      ]);
      clearInput();
