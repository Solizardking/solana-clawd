import { Check, Copy, Loader2, Monitor, Plus, Power, RefreshCw, Terminal } from "lucide-react";
...
function HostCard({ host, onRevoked }: { host: Host; onRevoked: (id: string) => void }) {
        ...
        <div className="flex items-center gap-2.5 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-1.5">
              <p className="font-medium text-xs truncate">{host.name || "Unnamed Device"}</p>
              <span
                className={cn(
...
function ConnectDeviceView({ onDone, onCancel }: { onDone: () => void; onCancel: () => void }) {
    ...
    return (
      <div className="p-4 space-y-4">
        <div className="text-center">
          ...
          <p className="text-[11px] text-muted-foreground mt-1">
            Run this command on the device you want to connect. We'll detect it automatically.
          </p>
        </div>
...
export function Devices() {
  ...
  const handleRevoked = (id: string) => {
    setHosts((prev) => prev.map((h) => (h.id === id ? { ...h, status: "revoked" } : h)));
  };
  ...
  const filtered =
    filter === "active"
      ? hosts.filter((h) => h.status === "active" || h.status === "pending_enrollment")
      : hosts;
    ...
    <div className="p-3 space-y-3">
      ...
      {loading ? (
        ...
        <div className="border border-dashed border-border rounded-sm py-10 text-center">
          ...
          <Button size="xs" variant="outline" onClick={() => setShowConnect(true)}>
            <Plus className="h-3 w-3" />
            Connect a Device
          </Button>
        </div>
