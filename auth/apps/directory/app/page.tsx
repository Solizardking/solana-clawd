...
export default async function LandingPage() {
  ...
  return (
    <div className="min-h-dvh flex flex-col">
        ...
        <div className="relative z-10 px-4 sm:px-6 lg:px-8 py-12 sm:py-20 lg:py-28 flex flex-col items-center text-center max-w-2xl mx-auto space-y-6">
          <div className="space-y-3">
            ...
            <p className="text-sm sm:text-base text-foreground/50 max-w-md mx-auto leading-relaxed">
              Discover Agent Auth-capable services by intent. A{" "}
              <a
      ...
      {/* Solana Agents — CAAP/1.0 verified agents */}
      <div className="relative px-4 sm:px-6 lg:px-8 py-10 sm:py-16 border-t border-foreground/[0.06]">
        <div className="flex items-center justify-between mb-6">
          <div className="space-y-1">
            ...
            <p className="text-xs text-foreground/30">
              Verified via{" "}
              <span className="font-mono text-foreground/50">CAAP/1.0</span> —
              Clawd Agent Attestation Protocol
            </p>
          </div>
          <Link
            href="https://x402.wtf/agentauth"
            target="_blank"
            rel="noopener noreferrer"
            className="text-[11px] font-mono text-foreground/35 hover:text-foreground/60 transition-colors"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            },
            {
              ...
              wallet: "AQoKYV7tYpTrFZN6P5oUoUCE6ec9tiWeo5eWAjgphNt8",
              tier: "Gold",
              clawd: "1.2M",
              protocol: "CAAP/1.0",
            },
            {
              name: "clawd-oracle-bot",
              wallet: "DRpbCBMxVnDK7maPGv7DErFVqgRJrm9FZgaVG4eLaY4G",
              tier: "Bronze",
              clawd: "120K",
              protocol: "CAAP/1.0",
            },
            ...
            <div
              key={agent.name}
              className="border border-foreground/[0.10] bg-foreground/[0.02] hover:bg-foreground/[0.04] transition-colors p-4 flex flex-col gap-2"
            >
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono text-foreground/70">{agent.name}</span>
                <span className="text-[10px] font-mono text-foreground/40 border border-foreground/[0.12] px-2 py-0.5">
                  {agent.tier}
                </span>
              </div>
              ...
              <div className="flex items-center gap-3 mt-1">
                <span className="text-[10px] text-foreground/30">
                  <span className="text-foreground/50">{agent.clawd}</span> CLAWD
                </span>
                <span className="text-[10px] font-mono text-foreground/20">{agent.protocol}</span>
              </div>
            </div>
          ))}
        </div>

        <div className="mt-6 flex items-center gap-2 text-[10px] font-mono text-foreground/25">
          <span>
            Attestation powered by{" "}
            <Link
              href="https://x402.wtf/agentauth"
              target="_blank"
              rel="noopener noreferrer"
              className="text-foreground/40 hover:text-foreground/60 transition-colors underline underline-offset-2"
            >
              @clawd/agent-auth-solana
            </Link>
          </span>
          <span className="text-foreground/[0.08]">·</span>
          <span>SIWS + Helius DAS + CLAWD SPL</span>
        </div>
      </div>
