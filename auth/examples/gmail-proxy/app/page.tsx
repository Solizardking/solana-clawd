  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-6 bg-black p-8 text-white">
      <div className="flex flex-col items-center gap-1">
        <span className="text-4xl font-bold tracking-tight">CLAWD MCP</span>
        <span className="text-sm text-zinc-400">
          CLAWD-attested MCP proxy · CAAP/1.0 · SIWS
        </span>
      </div>
      ...
      {session ? (
        <div className="flex w-full max-w-sm flex-col gap-3">
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-4">
            ...
            <ul className="space-y-2">
              {[
                { cap: "mcp.tools.list", req: "free", desc: "List available tools" },
                { cap: "mcp.tools.call", req: "bronze", desc: "Execute any tool" },
                { cap: "mcp.attest", req: "free", desc: "CAAP attestation check" },
                { cap: "mcp.discovery", req: "free", desc: "CAAP/1.0 discovery doc" },
              ].map(({ cap, req, desc }) => (
                <li key={cap} className="flex items-center justify-between text-sm">
                  <span>
                    <code className="text-xs text-orange-400">{cap}</code>
          ...
          <div className="rounded-xl border border-dashed border-zinc-800 bg-zinc-950 p-4 font-mono text-xs text-zinc-500">
            <p className="mb-1 text-zinc-600"># Start MCP server</p>
            <p>auth-agent mcp --url {"{your-caap-endpoint}"}</p>
          </div>
        ...
        <div className="flex flex-col items-center gap-4 rounded-xl border border-zinc-800 bg-zinc-900 p-8">
          <p className="text-sm text-zinc-400">
            Sign in with Solana to access CLAWD-attested MCP tools.
          </p>
          <button
            type="button"
            onClick={handleSignIn}
            className="rounded-lg bg-orange-600 px-6 py-2.5 text-sm font-semibold hover:bg-orange-500"
          >
            Sign In With Solana
          </button>
      ...
      <p className="text-xs text-zinc-600">
        CAAP/1.0 · SIWS · $CLAWD tiers · No Google OAuth · No passkeys
      </p>
    </main>
  );
}
